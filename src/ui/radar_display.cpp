...
bool tagRectangleInsideDisplay(int left, int top, int right, int bottom) {
  const int radius = radar::kCenterX - 1;
  const int radius_sq = radius * radius;
  const int x[] = {left, right, left, right};
  const int y[] = {top, top, bottom, bottom};
  for (size_t i = 0; i < 4; ++i) {
    const int dx = x[i] - radar::kCenterX;
    const int dy = y[i] - radar::kCenterY;
...
TagPlacement placeAircraftTag(int x, int y,
  ...
  const int clearance = symbol_half + radar::kAircraftLabelGapPx;
  const int right = x + clearance;
  const int left = x - clearance - block_w;
  const int above = y - clearance - block_h;
  const int below = y + clearance;
  const int middle_x = x - block_w / 2;
  const int middle_y = y - block_h / 2;

  int candidate_left[8];
  int candidate_top[8];
  const bool prefer_right = x < radar::kCenterX;
  candidate_left[0] = prefer_right ? right : left;
  candidate_top[0] = middle_y;
  candidate_left[1] = middle_x;
  candidate_top[1] = above;
  candidate_left[2] = middle_x;
  candidate_top[2] = below;
  candidate_left[3] = prefer_right ? left : right;
  candidate_top[3] = middle_y;
  candidate_left[4] = right;
  candidate_top[4] = above;
  candidate_left[5] = left;
  candidate_top[5] = above;
  candidate_left[6] = right;
  candidate_top[6] = below;
  candidate_left[7] = left;
  candidate_top[7] = below;

  TagPlacement best = {};
  int best_penalty = 0x7FFFFFFF;
  for (size_t i = 0; i < 8; ++i) {
    const int placed_left =
        std::max(1, std::min(candidate_left[i], radar::kSize - block_w - 1));
    const int placed_top =
        std::max(1, std::min(candidate_top[i], radar::kSize - block_h - 1));
    const int placed_right = placed_left + block_w;
    const int placed_bottom = placed_top + block_h;
    const int gap = radar::kAircraftTagCollisionGapPx;
    int penalty = 100 * runway::tagRectanglePenalty(
        placed_left - gap, placed_top - gap, placed_right + gap,
        placed_bottom + gap);
    const TagPlacement candidate = {placed_left, placed_top, placed_left,
                                    placed_right, placed_bottom};
    for (size_t other = 0; other < placed_count; ++other) {
      if (tagPlacementsTouch(candidate, placed[other])) {
        penalty += 100;
      }
    }
    if (!tagRectangleInsideDisplay(placed_left, placed_top, placed_right,
                                   placed_bottom)) {
      penalty += 1000;
    }
...
void drawAircraft() {
  ...
  for (size_t d = 0; d < draw_count; ++d) {
    const size_t i = items[d].index;
    placements[d] =
        placeAircraftTag(items[d].x, items[d].y, planes[i], placements, d);
    parents[d] = d;
  }
